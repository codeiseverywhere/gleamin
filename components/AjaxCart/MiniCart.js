(window.SpaceStation.AjaxCart.MiniCart = function (AjaxCart, $, Behaviors, betteroffcanvas, Shopify) {
    ...
    MiniCart.prototype.update_cart = function (data) {
        //general
        data.offcanvas_target_id = this.$elem.attr('id')
        data.money_format = this.money_format;
        data.header_height = 118;
        //subscriptions
        data.has_subscriptions = false;
        //shipping bar
        data.show_free_shipping = false;
        data.show_free_shipping_settings = document.getElementById('MainContent').dataset.freeshippingbar;
        if (data.show_free_shipping_settings == 'true') {
            data.show_free_shipping = true;
        }
        data.freeShippingThreshold = document.getElementById('MainContent').dataset.freeshippingthreshold;
        data.has_free_shipping = false;
        data.amount_remaining__free_shipping = 0;
        data.percent_to_free_shipping = 0;
        data.currency = document.getElementById('MainContent').dataset.currency;
        const cur = "USD";

        //Subscriptions
        for (var i = 0; i < data.items.length; i += 1) {
            for (var k in data.items[i].properties) {
                if (data.items[i].properties.hasOwnProperty(k) && k == "shipping_interval_frequency") {
                    data.has_subscriptions = true;
                }
            }
        }

        //Free Shipping
        if (data.total_price > data.freeShippingThreshold) {
            data.has_free_shipping = true;
        } else {
            data.amount_remaining__free_shipping = data.freeShippingThreshold - data.total_price;
        }
        data.percent_to_free_shipping = (data.total_price / data.freeShippingThreshold) * 100;
        if (data.currency != cur) {
            data.show_free_shipping = false;
        }
        //set header height with no shipping bar
        if (data.show_free_shipping == false) {
            data.header_height = 52;
        }
